...
const Home = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post("https://nexus-two-ivory.vercel.app/upload/single", singleFormData);
      await axios.post(
        "https://nexus-two-ivory.vercel.app/upload/multiple",
        multipleFormData
      );
    } catch (error) {
      console.error(error);
    ...
    <>
      <div className="bg-[#2A2828] w-screen h-[100vh] overflow-auto font-loaderend1">
        ...
        <div className="input_data w-full mt-6 md:mt-5">
          <div className="md:px-[30vw] px-[20vw]">
            <input
              ...
              type="text"
              className="w-full h-[6vw] md:h-[2.75vw] mb-2 md:mb-4 pl-4 outline-none border-white border-2 bg-transparent md:rounded-lg rounded-md text-white"
        ...
        <div className="get_sorted w-full items-center flex justify-center mt-3">
          ...
          >
            <Link to="/output">Get Sorted</Link>
          </button>
        </div>
      </div>
    </>
